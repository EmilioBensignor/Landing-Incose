<template>
    <div class="w-full xxl:max-w-[81.5rem] relative lg:px-16 xxl:px-0" ref="carouselWrapper">
        <button v-if="showLeftArrow" @click="scrollLeft"
            class="w-12 h-12 hidden md:flex justify-center items-center absolute z-10 bg-dark shadow-md rounded-full text-white hover:text-primary transition-colors duration-300"
            :style="leftButtonStyle" :disabled="isAtStart" aria-label="Anterior">
            <Icon name="material-symbols:chevron-left-rounded" size="48px" />
        </button>
        ...
        <div ref="container" class="scrollbar-hide overflow-x-auto cursor-grab select-none"
            ...
            <div class="carousel-wrapper flex pb-2 px-1 first:ml-5 md:first:ml-0 first:mr-5 md:first:mr-0"
                :style="wrapperStyles">
                <slot />
            </div>
        </div>
        ...
        <button v-if="showRightArrow" @click="scrollRight"
            ...
            <Icon name="material-symbols:chevron-right-rounded" size="48px" />
        </button>
    </div>
</template>
...
const responsiveLeft = computed(() => {
    ...
    return leftConfig?.[currentBreakpoint.value] || leftConfig?.base || '8px'
})
...
const responsiveRight = computed(() => {
    ...
    return rightConfig?.[currentBreakpoint.value] || rightConfig?.base || '8px'
})
